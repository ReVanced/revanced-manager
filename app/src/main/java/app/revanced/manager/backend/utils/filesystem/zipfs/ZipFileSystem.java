...
import static app.revanced.manager.backend.utils.filesystem.zipfs.ZipConstants.*;
import static app.revanced.manager.backend.utils.filesystem.zipfs.ZipUtils.*;
...
class ZipFileSystem extends FileSystem {
    ...
    private int getDefaultCompressionMethod(Map<String, ?> env) {
        int result =
                isTrue(env, "noCompression") ? ZipConstants.METHOD_STORED : ZipConstants.METHOD_DEFLATED;
                ...
                if (compressionMethod instanceof String) {
                    switch (((String) compressionMethod).toUpperCase()) {
                        case COMPRESSION_METHOD_STORED:
                            result = ZipConstants.METHOD_STORED;
                            break;
                        case COMPRESSION_METHOD_DEFLATED:
                            result = ZipConstants.METHOD_DEFLATED;
                            break;
                        default:
        ...
        try {
            ...
            if (inode == null) {
                ...
                Entry e = supportPosix ?
                    new PosixEntry(inode.name, inode.isdir, ZipConstants.METHOD_STORED) :
    ...
    void createDirectory(byte[] dir, FileAttribute<?>... attrs) throws IOException {
        ...
        try {
            ...
            checkParents(dir);
            Entry e = supportPosix ?
                new PosixEntry(dir, Entry.NEW, true, ZipConstants.METHOD_STORED, attrs) :
                new Entry(dir, Entry.NEW, true, ZipConstants.METHOD_STORED, attrs);
            update(e);
        } finally {
            endWrite();
        }
    ...
    private class EntryOutputChannel extends ByteArrayChannel {
        ...
        EntryOutputChannel(Entry e) {
            ...
            e.flag = ZipConstants.FLAG_DATADESCR;
            if (zc.isUTF8())
                e.flag |= ZipConstants.FLAG_USE_UTF8;
        }
        ...
        throws IOException
    {
        ...
        if (options.contains(StandardOpenOption.WRITE) ||
            ...
            try {
                ...
                try (InputStream is = getInputStream(e)) {
                    // TBD: if (e.size < NNNNN);
                    return new ByteArrayChannel(StreamUtils.readAllBytes(is), true);
                }
            } finally {
                endRead();
            }
        ...
        throws IOException
    {
        ...
        try {
            ...
            if (forWrite) {
                u.flag = ZipConstants.FLAG_DATADESCR;
    ...
    private END findEND() throws IOException {
        byte[] buf = new byte[ZipConstants.READBLOCKSZ];
        long ziplen = ch.size();
        long minHDR = (ziplen - ZipConstants.END_MAXLEN) > 0 ? ziplen - ZipConstants.END_MAXLEN : 0;
        long minPos = minHDR - (buf.length - ZipConstants.ENDHDR);

        for (long pos = ziplen - buf.length; pos >= minPos; pos -= (buf.length - ZipConstants.ENDHDR)) {
            int off = 0;
            if (pos < 0) {
            ...
            for (int i = buf.length - ZipConstants.ENDHDR; i >= 0; i--) {
                if (buf[i]   == (byte)'P'    &&
                    buf[i+1] == (byte)'K'    &&
                    buf[i+2] == (byte)'\005' &&
                    buf[i+3] == (byte)'\006' &&
                    (pos + i + ZipConstants.ENDHDR + ZipConstants.ENDCOM(buf, i) == ziplen)) {
                    // Found END header
                    buf = Arrays.copyOfRange(buf, i, i + ZipConstants.ENDHDR);
                    END end = new END();
                    ...
                    byte[] loc64 = new byte[ZipConstants.ZIP64_LOCHDR];
                    if (end.endpos < ZipConstants.ZIP64_LOCHDR ||
                        readFullyAt(loc64, 0, loc64.length, end.endpos - ZipConstants.ZIP64_LOCHDR)
                        != loc64.length ||
                        !ZipConstants.locator64SigAt(loc64, 0)) {
                        return end;
                    }
                    long end64pos = ZipConstants.ZIP64_LOCOFF(loc64);
                    byte[] end64buf = new byte[ZipConstants.ZIP64_ENDHDR];
                    if (readFullyAt(end64buf, 0, end64buf.length, end64pos)
                        != end64buf.length ||
                        !ZipConstants.end64SigAt(end64buf, 0)) {
                        return end;
                    }
                    // end64 found,
                    long cenlen64 = ZipConstants.ZIP64_ENDSIZ(end64buf);
                    long cenoff64 = ZipConstants.ZIP64_ENDOFF(end64buf);
                    long centot64 = ZipConstants.ZIP64_ENDTOT(end64buf);
                    // double-check
                    if (cenlen64 != end.cenlen && end.cenlen != ZipConstants.ZIP64_MINVAL ||
                        cenoff64 != end.cenoff && end.cenoff != ZipConstants.ZIP64_MINVAL ||
                        centot64 != end.centot && end.centot != ZipConstants.ZIP64_MINVAL32) {
                        return end;
                    }
    ...
    private byte[] initCEN() throws IOException {
        ...
        byte[] cen = new byte[(int)(end.cenlen + ZipConstants.ENDHDR)];
        if (readFullyAt(cen, 0, cen.length, cenpos) != end.cenlen + ZipConstants.ENDHDR) {
            throw new ZipException("read CEN tables failed");
        }
        // Iterate through the entries in the central directory
        inodes = new LinkedHashMap<>(end.centot + 1);
        int pos = 0;
        int limit = cen.length - ZipConstants.ENDHDR;
        while (pos < limit) {
            ...
            int flag   = ZipConstants.CENFLG(cen, pos);
            if ((flag & 1) != 0) {
                throw new ZipException("invalid CEN header (encrypted entry)");
            }
            if (method != ZipConstants.METHOD_STORED && method != ZipConstants.METHOD_DEFLATED) {
                throw new ZipException("invalid CEN header (unsupported compression method: " + method + ")");
            }
            if (pos + ZipConstants.CENHDR + nlen > limit) {
            ...
            if (zc.isUTF8() || (flag & ZipConstants.FLAG_USE_UTF8) != 0) {
        ...
        if (pos + ZipConstants.ENDHDR != cen.length) {
            throw new ZipException("invalid CEN header (bad header size)");
        }
        buildNodeTree();
        return cen;
        ...
        throws IOException
    {
        ...
        long size = 0;
        //  if there is A ext
        if ((e.flag & ZipConstants.FLAG_DATADESCR) != 0) {
            if (e.size >= ZipConstants.ZIP64_MINVAL || e.csize >= ZipConstants.ZIP64_MINVAL)
                size = 24;
            else
                size = 16;
        }
        ...
        if (readFullyAt(buf, 0, ZipConstants.LOCHDR + 1 , locoff) != ZipConstants.LOCHDR + 1)
            throw new ZipException("loc: reading failed");

        if (updateHeader || ZipConstants.LOCNAM(buf) > 0 && buf[ZipConstants.LOCHDR] == '/') {
            locoff += ZipConstants.LOCHDR + ZipConstants.LOCNAM(buf) + ZipConstants.LOCEXT(buf);  // skip header
            size += e.csize;
            written = e.writeLOC(os) + size;
        } else {
            os.write(buf, 0, ZipConstants.LOCHDR);    // write out the loc header
            locoff += ZipConstants.LOCHDR;
            // use e.csize,  LOCSIZ(buf) is zero if FLAG_DATADESCR is on
            // size += LOCNAM(buf) + LOCEXT(buf) + LOCSIZ(buf);
            size += ZipConstants.LOCNAM(buf) + ZipConstants.LOCEXT(buf) + e.csize;
            written = ZipConstants.LOCHDR + size;
        }
        int n;
        while (size > 0 &&
    ...
    private long writeEntry(Entry e, OutputStream os)
        ...
        long written = 0;
        if (e.method != ZipConstants.METHOD_STORED && e.csize > 0 && (e.crc != 0 || e.size == 0)) {
            // pre-compressed entry, write directly to output stream
            writeTo(e, os);
        } else {
            try (OutputStream os2 = (e.method == ZipConstants.METHOD_STORED) ?
    ...
    private OutputStream getOutputStream(Entry e) throws IOException {
        ...
        if (e.method == ZipConstants.METHOD_DEFLATED) {
        ...
        throws IOException
    {
        ...
        } else if (e.method == ZipConstants.METHOD_STORED) {
    ...
    static class END {
        ...
        void write(OutputStream os, long offset, boolean forceEnd64) throws IOException {
            ...
            long xlen = cenlen;
            long xoff = cenoff;
            if (xlen >= ZipConstants.ZIP64_MINVAL) {
                xlen = ZipConstants.ZIP64_MINVAL;
                hasZip64 = true;
            }
            if (xoff >= ZipConstants.ZIP64_MINVAL) {
                xoff = ZipConstants.ZIP64_MINVAL;
                hasZip64 = true;
            }
            int count = centot;
            if (count >= ZipConstants.ZIP64_MINVAL32) {
                count = ZipConstants.ZIP64_MINVAL32;
                hasZip64 = true;
            }
            if (hasZip64) {
                //zip64 end of central directory record
                writeInt(os, ZipConstants.ZIP64_ENDSIG);       // zip64 END record signature
                writeLong(os, ZipConstants.ZIP64_ENDHDR - 12); // size of zip64 end
                ...
                writeInt(os, ZipConstants.ZIP64_LOCSIG);       // zip64 END locator signature
                writeInt(os, 0);                  // zip64 END start disk
                writeLong(os, offset);            // offset of zip64 END
                writeInt(os, 1);                  // total number of disks (?)
            }
            writeInt(os, ZipConstants.ENDSIG);                 // END record signature
    ...
    static class IndexNode {
        ...
        IndexNode(byte[] cen, int pos, int nlen) {
            int noff = pos + ZipConstants.CENHDR;
            if (cen[noff + nlen - 1] == '/') {
                isdir = true;
                nlen--;
            }
    ...
    static class Entry extends IndexNode implements ZipFileAttributes {
        ...
        Entry(byte[] name, Path file, int type, FileAttribute<?>... attrs) {
            this(name, type, false, ZipConstants.METHOD_STORED, attrs);
            this.file = file;
        }
        ...
        private int version(boolean zip64) throws ZipException {
            ...
            if (method == ZipConstants.METHOD_DEFLATED)
                return 20;
            else if (method == ZipConstants.METHOD_STORED)
                return 10;
        ...
        private int versionMadeBy(int version) {
            return (posixPerms < 0) ? version :
                ZipConstants.VERSION_MADE_BY_BASE_UNIX | (version & 0xff);
        }
        ...
        private void readCEN(ZipFileSystem zipfs, IndexNode inode) throws IOException {
            ...
            if (!ZipConstants.cenSigAt(cen, pos))
                throw new ZipException("invalid CEN header (bad signature)");
            version     = ZipConstants.CENVER(cen, pos);
            flag        = ZipConstants.CENFLG(cen, pos);
            method      = ZipConstants.CENHOW(cen, pos);
            mtime       = dosToJavaTime(ZipConstants.CENTIM(cen, pos));
            crc         = ZipConstants.CENCRC(cen, pos);
            csize       = ZipConstants.CENSIZ(cen, pos);
            size        = ZipConstants.CENLEN(cen, pos);
            int nlen    = ZipConstants.CENNAM(cen, pos);
            int elen    = ZipConstants.CENEXT(cen, pos);
            int clen    = ZipConstants.CENCOM(cen, pos);
            /*
            ...
            */
            if (ZipConstants.CENVEM_FA(cen, pos) == ZipConstants.FILE_ATTRIBUTES_UNIX) {
                posixPerms = ZipConstants.CENATX_PERMS(cen, pos) & 0xFFF; // 12 bits for setuid, setgid, sticky + perms
            }
            locoff      = ZipConstants.CENOFF(cen, pos);
        ...
        private int writeCEN(OutputStream os) throws IOException {
            ...
            if (csize >= ZipConstants.ZIP64_MINVAL) {
                csize0 = ZipConstants.ZIP64_MINVAL;
                elen64 += 8;                 // csize(8)
            }
            if (size >= ZipConstants.ZIP64_MINVAL) {
                size0 = ZipConstants.ZIP64_MINVAL;        // size(8)
                elen64 += 8;
            }
            if (locoff >= ZipConstants.ZIP64_MINVAL) {
                locoff0 = ZipConstants.ZIP64_MINVAL;
                elen64 += 8;                 // offset(8)
            }
            if (elen64 != 0) {
            ...
            while (eoff + 4 < elen) {
                int tag = ZipConstants.SH(extra, eoff);
                int sz = ZipConstants.SH(extra, eoff + 2);
                if (tag == ZipConstants.EXTID_EXTT || tag == ZipConstants.EXTID_NTFS) {
                    foundExtraTime = true;
                }
                eoff += (4 + sz);
            }
            ...
            writeInt(os, ZipConstants.CENSIG);            // CEN header signature
            ...
            if (zip64) {
                writeShort(os, ZipConstants.EXTID_ZIP64);// Zip64 extra
                writeShort(os, elen64 - 4); // size of "this" extra block
                if (size0 == ZipConstants.ZIP64_MINVAL)
                    writeLong(os, size);
                if (csize0 == ZipConstants.ZIP64_MINVAL)
                    writeLong(os, csize);
                if (locoff0 == ZipConstants.ZIP64_MINVAL)
                    writeLong(os, locoff);
            }
            if (elenNTFS != 0) {
                writeShort(os, ZipConstants.EXTID_NTFS);
            ...
            if (elenEXTT != 0) {
                writeShort(os, ZipConstants.EXTID_EXTT);
            ...
            return ZipConstants.CENHDR + nlen + elen + clen + elen64 + elenNTFS + elenEXTT;
        }
        ...
        private int writeLOC(OutputStream os) throws IOException {
            ...
            int elenEXTT = 0;
            int elenNTFS = 0;
            writeInt(os, ZipConstants.LOCSIG);               // LOC header signature
            if ((flag & ZipConstants.FLAG_DATADESCR) != 0) {
                ...
                if (csize >= ZipConstants.ZIP64_MINVAL || size >= ZipConstants.ZIP64_MINVAL) {
                ...
                writeInt(os, crc);              // crc-32
                if (zip64) {
                    writeInt(os, ZipConstants.ZIP64_MINVAL);
                    writeInt(os, ZipConstants.ZIP64_MINVAL);
                } else {
            ...
            writeBytes(os, zname, 1, nlen);
            if (zip64) {
                writeShort(os, ZipConstants.EXTID_ZIP64);
                writeShort(os, 16);
                writeLong(os, size);
                writeLong(os, csize);
            }
            ...
            return ZipConstants.LOCHDR + nlen + elen + elen64 + elenNTFS + elenEXTT;
        }

        // Data Descriptor
        private int writeEXT(OutputStream os) throws IOException {
            writeInt(os, ZipConstants.EXTSIG);           // EXT header signature
            writeInt(os, crc);              // crc-32
            if (csize >= ZipConstants.ZIP64_MINVAL || size >= ZipConstants.ZIP64_MINVAL) {
                writeLong(os, csize);
                writeLong(os, size);
        ...
        private void readExtra(ZipFileSystem zipfs) throws IOException {
            ...
            while (off + 4 < elen) {
                ...
                int pos = off;
                int tag = ZipConstants.SH(extra, pos);
                int sz = ZipConstants.SH(extra, pos + 2);
                pos += 4;
                ...
                case ZipConstants.EXTID_ZIP64 :
                    if (size == ZipConstants.ZIP64_MINVAL) {
                        ...
                        size = ZipConstants.LL(extra, pos);
                        pos += 8;
                    }
                    if (csize == ZipConstants.ZIP64_MINVAL) {
                        if (pos + 8 > elen)
                            break;
                        csize = ZipConstants.LL(extra, pos);
                        pos += 8;
                    }
                    if (locoff == ZipConstants.ZIP64_MINVAL) {
                        if (pos + 8 > elen)
                            break;
                        locoff = ZipConstants.LL(extra, pos);
                    }
                    break;
                case ZipConstants.EXTID_NTFS:
                    ...
                    if (ZipConstants.SH(extra, pos) !=  0x0001)
                        break;
                    if (ZipConstants.SH(extra, pos + 2) != 24)
                        break;
                    // override the loc field, datatime here is
                    // more "accurate"
                    mtime  = winToJavaTime(ZipConstants.LL(extra, pos + 4));
                    atime  = winToJavaTime(ZipConstants.LL(extra, pos + 12));
                    ctime  = winToJavaTime(ZipConstants.LL(extra, pos + 20));
                    break;
                case ZipConstants.EXTID_EXTT:
                    ...
                    if (zipfs.noExtt) {
                        if (sz == 5)
                            mtime = unixToJavaTime(ZipConstants.LG(extra, pos + 1));
                         break;
                    }
        ...
        private void readLocEXTT(ZipFileSystem zipfs) throws IOException {
            ...
            if (!ZipConstants.locSigAt(buf, 0))
                throw new ZipException("R"
                                   + Long.toString(ZipConstants.getSig(buf, 0), 16));
            int locElen = ZipConstants.LOCEXT(buf);
            if (locElen < 9)    // EXTT is at least 9 bytes
                return;
            int locNlen = ZipConstants.LOCNAM(buf);
            buf = new byte[locElen];
            if (zipfs.readFullyAt(buf, 0, buf.length , locoff + ZipConstants.LOCHDR + locNlen)
                != buf.length)
            ...
            while (locPos + 4 < buf.length) {
                ...
                locPos += 4;
                if (locTag  != ZipConstants.EXTID_EXTT) {
                    locPos += locSZ;
                     continue;
                }
                int end = locPos + locSZ - 4;
                int flag = ZipConstants.CH(buf, locPos++);
                if ((flag & 0x1) != 0 && locPos <= end) {
                    mtime = unixToJavaTime(ZipConstants.LG(buf, locPos));
                    locPos += 4;
                }
                if ((flag & 0x2) != 0 && locPos <= end) {
                    atime = unixToJavaTime(ZipConstants.LG(buf, locPos));
                    locPos += 4;
                }
                if ((flag & 0x4) != 0 && locPos <= end) {
                    ctime = unixToJavaTime(ZipConstants.LG(buf, locPos));
                }
                break;
            }
        }

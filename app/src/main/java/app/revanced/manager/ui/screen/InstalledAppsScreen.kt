...
fun InstalledAppsScreen(
    ...
    val suggestedVersions by selectorVm.suggestedAppVersions.collectAsStateWithLifecycle()

    selectorVm.nonSuggestedVersionDialogSubject?.let {
        NonSuggestedVersionDialog(
            suggestedVersion = suggestedVersions[it.packageName].orEmpty(),
            onDismiss = selectorVm::dismissNonSuggestedVersionDialog
        )
    }
    ...
    if (search) {
        ...
        val patchedPackageNames = installedApps?.mapTo(mutableSetOf()) { it.currentPackageName }.orEmpty()
                ...
                LazyColumnWithScrollbar(modifier = Modifier.fillMaxSize()) {
                    items(
                        ...
                        ListItem(
                            ...
                            trailingContent = app.patches?.let { patchCount ->
                                {
                                    Text(
    ...
    LazyColumnWithScrollbar(
        ...
        val patchedPackageNames = patched.mapTo(mutableSetOf()) { it.currentPackageName }
        val visiblePatchableApps = patchable.filter { it.packageName !in patchedPackageNames }

        item(key = "HEADER_PATCHED") {
            GroupHeader(stringResource(R.string.patched_apps_section_title))
        }
        ...
        items(
            ...
            ListItem(
                ...
                trailingContent = if (patchCount != null) {
                    {
                        Text(
                            pluralStringResource(
                                R.plurals.patch_count,
